...
class Admin_Categories extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->validation_rules = array(
			...
			array(
				'field' => 'position',
				'label' => lang('category_position_label'),
				'rules' => 'trim|numeric'
			),
		);
	...
	public function index()
	{
		...
		//$this->data->pagination = create_pagination('admin/blog/categories/index', $total_rows);
			
		// Using this data, get the relevant results
		$this->data->categories =$this->blog_categories_m->get_list_categories(0);//->limit( $this->data->pagination['limit'] )->get_all();
	...
	public function create()
	{
		...
		// Validate the data
		$this->validation_rules[1]['rules'] .= '|callback__check_title';
		
		if ($this->form_validation->run())
		{
			$this->blog_categories_m->insert($_POST)
				? $this->session->set_flashdata('success', sprintf( lang('cat_add_success'), $this->input->post('name')) )
		...
		$this->data->parent_category =$this->blog_categories_m->get_list_categories(0);		
	...
	{	
		...
		{		
			$this->blog_categories_m->update($id, $_POST)
				? $this->session->set_flashdata('success', sprintf( lang('cat_edit_success'), $this->input->post('name')) )
		...
		// Render the view
		$this->data->category =& $category;
		$this->template->title($this->module_details['name'], sprintf(lang('cat_edit_title'), $category->name))
						->build('admin/categories/form', $this->data);
	}	
	...
	public function _check_title($name = '') {
		if (!$this->blog_categories_m->check_title($name)) {
